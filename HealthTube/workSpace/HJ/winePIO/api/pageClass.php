...
class paging{
	...
	public function showPage(){
		...
		if($this->display_mode && ($this->page != 1)){
			...
			$this->html =  "<li class='page-item'><a href=\"http://{$this->url}page=1\">{$this->start_btn}</a> </li>";
		}
		if($this->is_prev){
		...
		for($i = $this->start_page; $i <= $this->end_page; $i++){
			if($i == $this->page){	
				...
				$this->html .= "<li class='page-item'><span>$i</span></li>";
			}else{					
				//$this->html .= " <a href=\"http://{$this->url}page=$i\"{$this->display_class}{$this->display_id}>{$i}</a> ";
				$this->html .= " <li class='page-item'><a href=\"http://{$this->url}page=$i\"{$this->display_class}{$this->display_id}>{$i}</a></li> ";
			}
		}
