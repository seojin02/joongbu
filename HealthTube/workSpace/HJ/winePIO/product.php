...
//검색
	...
	if(isset($searchColumn) && isset($searchText)) {
		$searchSql = ' where flag = 1 and ' . $searchColumn . ' like "%' . $searchText . '%"';
	} else {
	...
	$query = "SELECT count(*) FROM pioWineT ".$searchSql;
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
	$cnt = $conn->DBF();
	...
	$query ="SELECT * FROM pioWineT ".$searchSql." ORDER BY idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
	...
	<?$layout->sideMenu($sideMenu);?>
		...
		<section id="main-content" style="min-height:800px;">
			<section class="wrapper">
				<div class="row">
					<div class="col-lg-6">
						<h3><i class="fa fa-angle-right"></i><a href="<?=$_SERVER['PHP_SELF']?>">와인 관리</a></h3>
					</div>
				</div>
							...
							<div class="input-group">
								<div class="input-group-btn search-panel">
									<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
										<span id="search_concept">와인검색</span></span>
									</button>
								</div>
							...
							<section id="no-more-tables">
								<table class="table table-bordered table-hover table-striped">
									<thead class="cf" style='background-color: #BDBDBD'>
										...
										<th>상품코드</th>
										<th>상품이름</th>
										<th class="numeric">Dry</th>
										<th class="numeric">Tannin</th>
										<th class="numeric">Acidity</th>
										<th class="numeric">Body</th>
										</tr>
									</thead>
									<tbody>
										...
										<tr>
											...
											<td data-title="상품코드">
												<a href="product_form.php?no=<?echo $row ['idx'];?>">
												<?echo 'I'.$row['idx'];?></a>
											</td>
											<td data-title="상품이름">
												<a href="product_form.php?no=<?echo $row ['idx'];?>">
											...
											<td class="numeric" data-title="Dry"><?echo $row['Dry'];?></td>
											<td class="numeric" data-title="Tannin"><?echo $row['Tannin'];?></td>
											<td class="numeric" data-title="Acidity"><?echo $row['Acidity'];?></td>
											<td class="numeric" data-title="Body"><?echo $row['Body'];?></td>
										</tr>
										<?}
										}else{$empty = "결과가 없습니다."?>
										<?}?>
