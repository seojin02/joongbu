...
//검색
	...
	if(isset($searchColumn) && isset($searchText)) {
		$searchSql = ' where pioPurchaseT.flag = 1 and ' . $searchColumn . ' like "%' . $searchText . '%"';
	} else {
		$searchSql = ' where pioPurchaseT.flag = 1';
	}
	...
	$query = "SELECT count(*) 
              ...
              JOIN pioWineT
              ON pioWineT.idx = pioPurchaseT.wineIdx ".$searchSql;
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
	$cnt = $conn->DBF();
	...
	$query ="SELECT pioUserT.name as 'user', pioWineT.name as 'wine', pioPurchaseT.date as 'date', pioPurchaseT.eventId as 'eventId', pioPurchaseT.idx as 'idx' 
             FROM pioPurchaseT 
             JOIN pioUserT 
             ON pioUserT.idx = pioPurchaseT.userIdx
             JOIN pioWineT
             ON pioWineT.idx = pioPurchaseT.wineIdx ".$searchSql." ORDER BY pioPurchaseT.idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
	...
	<?$layout->sideMenu($sideMenu);?>
		...
		<section id="main-content" style="min-height:800px;">
			<section class="wrapper">
				<div class="row">
					<div class="col-lg-6">
						<h3><i class="fa fa-angle-right"></i><a href="<?=$_SERVER['PHP_SELF']?>">판매 관리</a></h3>
					</div>
				</div>
					...
					<div class="col-lg-12" style="">
						<form action="<?=$_SERVER['PHP_SELF']?>" method="get">
							<div class="input-group">
								...
								<input type="hidden" name="search_param" value="pioUserT.name" id="search_param">         
				...
				<form onsubmit="return validate();" action="./api/basicReg/companyD.php" method="post" name="store_form">
						...
						<div class="col-lg-12" style="" id="print"> 
							<section id="no-more-tables">
								<table class="table table-bordered table-hover table-striped">
									...
									<tbody>
										<?if($result!= 0){while($row = $conn->DBF()){ $eventId = mb_substr($row['eventId'],0,8);?>
										<tr>
											<td data-title="선택">
												<input type="checkbox" name="chk_info[]" value="<?echo $row ['idx'];?>">
											</td>
											<td data-title="판매코드">
												<a href="purchase_form.php?no=<?echo $row ['idx'];?>">
												<?echo $eventId;?></a>
											</td>
											<td data-title="고객명">
												<a href="purchase_form.php?no=<?echo $row ['idx'];?>">
					...
					<div class="row" style="text-align:right">
						<div class="col-lg-12" style=""> 
							<button type="button" class="btn btn-default" onclick="location.href='purchase_form.php'">신규</button>
...
<?
/*
SELECT pioUserT.name as 'user', pioWineT.name as 'wine', pioPurchaseT.date as 'date', pioPurchaseT.eventId as 'eventId' 
FROM pioPurchaseT 
JOIN pioUserT 
ON pioUserT.idx = pioPurchaseT.userIdx
JOIN pioWineT
ON pioWineT.idx = pioPurchaseT.wineIdx
WHERE pioPurchaseT.flag = 1
*/
?>