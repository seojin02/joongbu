...
//검색
	...
	$query = "SELECT count(*) FROM pioUserT ".$searchSql;
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
	$cnt = $conn->DBF();
	...
	$query ="SELECT * FROM pioUserT ".$searchSql." ORDER BY idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
								...
								<table class="table table-bordered table-hover table-striped">
									...
									<?if($result!= 0){while($row = $conn->DBF()){?>
										<tr>
											...
											<td data-title="고객코드">
												<a href="user_form.php?no=<?echo $row ['idx'];?>">
												<?echo 'U'.$row['idx'];?></a>
											</td>
											<td data-title="고객이름">
												<a href="user_form.php?no=<?echo $row ['idx'];?>">
