...
// 페이징
$query = "select * from Heal_video $searchText $searchPurpose $searchBody $searchEquipment and flag = '1' order by idx desc";
$conn->DBQ($query);
$conn->DBE();
$cnt = $conn->resultRow();
...
<section class="blog_area section_padding" style="min-height:1100px;padding: 100px 0px;">
	<div class="container">
		...
		<div class="row">
			...
			<?
			$sql = "
			select * from Heal_video $searchText $searchPurpose $searchBody $searchEquipment and flag = '1' order by idx desc limit $limit, $list;
			";
			$conn->DBQ($sql);
			$conn->DBE();
			while($row=$conn->DBF()){
				...
				<div class="col-lg-3 mt-3">
					...
					<?if($_SESSION['id'] == 'admin'){?>
						<div class="col-lg-12 text-right mb-2 mt-2">
							<form action="api/videoReg/delete.php" method="post" id="del<?echo $row['idx'];?>">
								<input type="hidden" name="no" value="<?php echo $row['idx']; ?>">
								<button type="button" class="btn btn-flat btn-danger btn-sm" id="delBtn<?echo $row['idx'];?>">X</button>
							</form>
							<script>
...
<!-- 					<nav class="blog-pagination justify-content-center d-flex"> -->
				...
				<?if($_SESSION['id'] == 'admin'){?>
					<div class="col-lg-12 text-right">
						<a href="youtubeApi.php"><button type="button" class="btn btn-flat btn-danger btn-md">영상 등록</button></a>
					</div>
				<?}?>
